...
@interface DoneViewController (){
    NSMutableArray *filteredDoneTasks,*filteredHighPriorityArray,*filteredMediumPriorityArray,*filteredLowPriorityArray;
...
- (void)viewDidLoad {
    ...
    filtered=false;
       ...
       filteredDoneTasks=[NSMutableArray new];
...
-(void)filterarray{
    [filteredDoneTasks removeAllObjects];
    for (NSDictionary*dic in myListOfTasks) {
        NSString *savedTaskStatus=[dic objectForKey:@"status"];
        if([savedTaskStatus isEqualToString:@"Done"]){
            [filteredDoneTasks addObject:dic];
        }
    }
...
-(void)filterPriorityOfTasks{
    if (filtered) {
        ...
        filtered=true;
          ...
          for (NSDictionary*dic in filteredDoneTasks) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if (filtered) {
        ...
        length=filteredDoneTasks.count;
        if (length==0) {
            [label setHidden:NO];
               tableView.alpha=0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    NSDictionary *task;
    DoneTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:@"DoneTableViewCell1" forIndexPath:indexPath];
    ...
    if (filtered) {
        switch (indexPath.section) {
          ...
          task=[filteredDoneTasks objectAtIndex:indexPath.row];
     }
